...
describe('HeadersInterceptor', () => {
  ...
  it('should add GUID header', () => {
    httpClient.get('/test').subscribe(response => expect(response).toBeTruthy());

    const httpRequest = httpMock.expectOne('/test');

    expect(httpRequest.request.headers.has('GUID')).toEqual(true);
    expect(httpRequest.request.headers.get('GUID')).toBe('f4179b26-21ac-432c-bcd8-cb4bc6e50981');
  });
